...
export class HttpService {
  ...
  public async ensureRoomExists(
    ...
    return await axios.get(
      `http://localhost:9090/room/ensure-room-exists/${roomId}`,
      { withCredentials: true }
    );
  }
  ...
  public fetchAllUsersInConversation(
    groupUrl: string
  ): Promise<AxiosResponse<GroupUserModel[]>> {
    return this.instance.get("users/group/" + groupUrl, {});
  }

  public fetchAllUsersWithoutAlreadyInGroup(
    groupUrl: string
  ): Promise<AxiosResponse<GroupUserModel[]>> {
    return this.instance.get("users/all/" + groupUrl, {});
  }

  public fetchAllUsersToAddInConversation(
    userId: number | string
  ): Promise<AxiosResponse<ConversationUserModel[]>> {
    return this.instance.get("users/all/conversation/" + userId);
  }
  ...
  public leaveConversation(
    ...
    return this.instance.get(
      "user/leave/" + userIdToRemove + "/group/" + groupId
    );
  }

  public removeUserFromConversation(
    userIdToRemove: string | number,
    groupUrl: string
  ): Promise<AxiosResponse> {
    return this.instance.get(
      "user/remove/" + userIdToRemove + "/group/" + groupUrl
    );
  }

  public removeAdminUserInConversation(
    userIdToRemove: string | number,
    groupUrl: string
  ): Promise<AxiosResponse> {
    return this.instance.get(
      "user/remove/admin/" + userIdToRemove + "/group/" + groupUrl
    );
  }

  public grantUserAdminInConversation(
    userIdToGrant: number | string,
    groupId: string
  ): Promise<AxiosResponse> {
    return this.instance.get(
      "user/grant/" + userIdToGrant + "/group/" + groupId
    );
  }
